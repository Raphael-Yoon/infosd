...
def upgrade(conn):
    # 1. Q7 어조 통일
    conn.execute(
        "UPDATE ipd_questions SET text='향후(차기 연도) 정보보호 투자 계획이 있나요?' WHERE id='Q7'"
    )

    # 2. Q9 후행 공백 제거
    conn.execute(
        "UPDATE ipd_questions SET text='정보보호 전담 부서 또는 전담 인력이 있나요?' WHERE id='Q9'"
    )

    # 3. Q13 질문형 전환
    conn.execute(
        "UPDATE ipd_questions SET text='최고책임자(CISO/CPO)가 지정되어 있나요?' WHERE id='Q13'"
    )
...
def downgrade(conn):
    conn.execute(
        "UPDATE ipd_questions SET text='향후(차기 연도) 정보보호 투자 계획이 있으신가요?' WHERE id='Q7'"
    )
    conn.execute(
        "UPDATE ipd_questions SET text='정보보호 전담 부서 또는 전담 인력이 있나요? ' WHERE id='Q9'"
    )
