...
- [x] ✅ **test_evidence_upload**: 허용 확장자(PNG) 파일 업로드 성공 확인 → **통과** (PNG 업로드 성공 (id: f8b4ac4a...))
- [x] ✅ **test_evidence_invalid_ext**: 비허용 확장자(exe) 파일 업로드 차단 확인 → **통과** (비허용 확장자 차단 확인 (status: 400))
- [x] ✅ **test_evidence_delete**: 업로드된 증빙 파일 삭제 API 정상 동작 확인 → **통과** (증빙 삭제 성공 (ID: 41735096...))
...
- [x] ✅ **test_investment_ratio_api**: Q2/Q4 답변 저장 후 I-3 투자비율(B/A) 자동계산 표시 확인 → **통과** (투자비율 자동계산 표시 확인 (20.00%))
- [x] ✅ **test_personnel_ratio_api**: Q10/Q28/Q11/Q12 답변 저장 후 II-4 인력비율(D/C) 자동계산 표시 확인 → **통과** (인력비율 자동계산 표시 확인 (35.00%))

## 11. table 타입 증빙 자료

- [x] ✅ **test_evidence_for_table_type**: table 타입 질문(Q29 CISO 활동내역) 증빙 파일(PDF) 업로드 및 삭제 확인 → **통과** (table 타입(Q29) 증빙 PDF 업로드 및 삭제 확인)

## 12. 증빙 섹션 표시 조건

- [~] ⚠️ **test_evidence_section_toggle_by_value**: number 타입 금액 0 입력 시 증빙 섹션 숨김, 양수 입력 시 표시 전환 확인 → **경고** (Q2=0 에도 증빙 섹션이 표시됨 (Q2에 evidence_list 없을 수도 있음))
